...
import { ChevronLeft, Sparkles, Droplets, Moon, Apple, Dumbbell, Users, Trophy } from "lucide-react";
...
function HomeV2() {
  ...
  const tier = tierFor(userV2.bodyIQ);
    ...
    <ShellV2 bare>
      ...
      <div className="px-5 mt-4 grid grid-cols-[auto_1fr] gap-3 items-center">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <StatTile icon={Droplets} label="ماء" value={`${userV2.waterCups}/${userV2.waterTarget}`} hint="كأس" color="var(--primary)" to="/sheila-v2/nutrition" />
          <StatTile icon={Apple} label="سعرات" value={`${userV2.caloriesToday}`} hint={`من ${userV2.caloriesTarget}`} color="var(--phase-menstrual)" to="/sheila-v2/nutrition" />
        </div>
      </div>

      {/* Block 4 — خطّة اليوم (تمرين + وجبة) */}
      <div className="px-5 mt-5">
        ...
        <div className="grid gap-2.5">
          <Link to="/sheila-v2/workouts/player" className="rounded-2xl p-3.5 bg-white/85 border border-border flex items-center gap-3 active:scale-[0.99] transition-transform">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0" style={{ background: `linear-gradient(135deg, ${meta.color}55, ${meta.color}22)` }}>
              <Dumbbell size={18} style={{ color: meta.color }} strokeWidth={1.9} />
            </div>
          ...
          <Link to="/sheila-v2/nutrition/meal/$id" params={{ id: meal.id }} className="rounded-2xl p-3.5 bg-white/85 border border-border flex items-center gap-3 active:scale-[0.99] transition-transform">
      ...
      {/* Block 5 — اختصار دائرة شيلا + تحدّيات */}
      <div className="px-5 mt-5">
        <Link to="/sheila-v2/circle" className="block rounded-2xl p-4 relative overflow-hidden"
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="text-[11px] text-foreground/65 mt-0.5">3 منشورات جديدة من مجموعاتك اليوم</div>
            </div>
      ...
      <div className="px-5 mt-4 grid grid-cols-4 gap-2.5">
        ...
        <Shortcut to="/sheila-v2/circle/accountability" icon={Users} label="شريك" color="var(--phase-follicular)" />
      </div>
...
function StatTile({ icon: Icon, label, value, hint, color, to }: any) {
  return (
    <Link to={to} className="rounded-2xl bg-white/85 border border-border p-3 active:scale-[0.97] transition-transform">
      ...
      <div className="font-display text-[18px] mt-1 leading-none" style={{ color }}>{value}</div>
