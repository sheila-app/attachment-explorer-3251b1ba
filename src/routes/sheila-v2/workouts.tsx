...
import { Sparkles, Wand2, ChevronLeft, Play } from "lucide-react";
...
import { userV2, PHASE_META, WORKOUTS } from "@/data/sheila-v2";
    ...
    <ShellV2 title="التمارين" rightSlot={<PhasePill phase={phase} name={meta.name} color={meta.color} />} variant="energetic">
      <div className="px-5">
        ...
        <div className="rounded-3xl p-5 glass-strong">
          <div className="relative z-10 flex items-center gap-2 mb-2"><Sparkles size={14} className="text-primary" /><span className="text-[11px] font-semibold text-primary">اقتراح شيلا</span></div>
          <Link to="/sheila-v2/workouts/player" className="relative z-10 block rounded-2xl bg-white/70 p-4 active:scale-[0.99] transition-transform">
        ...
        <div className="grid gap-2.5">
          {WORKOUTS.map((w) => {
            ...
            return (
              <Link key={w.id} to="/sheila-v2/workouts/player" className="rounded-2xl bg-white/85 border border-border p-3.5 flex items-center gap-3 active:scale-[0.99]">
                <div className="w-12 h-12 rounded-xl shrink-0 flex items-center justify-center" style={{ background: `linear-gradient(135deg, ${m.color}55, ${m.color}22)` }}>
                  <span className="text-[14px] font-display" style={{ color: m.deep }}>{w.duration}</span>
                </div>
